...
int main() {
    char filename[100];
    printf("Enter the filename with the maze: ");
    scanf("%s", filename);
    ...
    // Îïðåäåëÿåì ðàçìåðû ëàáèðèíòà
    int M = 0, N = 0;
    char line[100];
    while (fgets(line, sizeof(line), file)) {
        M++;
        N = strlen(line) - 1; // Óáèðàåì ñèìâîë íîâîé ñòðîêè
    }
    rewind(file);
    
    // Âûäåëÿåì ïàìÿòü ïîä ëàáèðèíò
    char** maze = (char**)malloc(M * sizeof(char*));
    int i;
    for (i = 0; i < M; i++) {
        maze[i] = (char*)malloc((N + 1) * sizeof(char));
        fgets(maze[i], N + 1, file);
        maze[i][N] = '\0'; // Óáåäèìñÿ, ÷òî ñòðîêà çàâåðøàåòñÿ íóë¸ì
    }
    fclose(file);
